      ...
      {activeTab === 'requests' && (
        <div className="space-y-4">
          <div className="flex justify-between items-center">
            ...
            <Button onClick={loadData} variant="outline">Обновить</Button>
          </div>
                ...
                <Card key={request.id} className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      {request.company && <p className="text-sm text-gray-500">Компания: {request.company}</p>}
                    </div>
                    <div className="text-right">
                      <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(request.status)}`}>
                        {request.status}
                      </span>
                      <p className="text-sm text-gray-500 mt-1">{formatDate(request.createdAt)}</p>
                    </div>
                  </div>
                  ...
                  <div className="flex space-x-2">
                    ...
                    <Button size="sm" variant="outline">Отметить как обработанную</Button>
                  </div>
                </Card>
              ))}
            </div>
                  ...
                  {products.map((product) => (
                    <tr key={product.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                        <span className={`${product.stock < 5 ? 'text-red-600' : 'text-green-600'}`}>{product.stock}</span>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                        ...
                        <Button size="sm" variant="outline">{product.isAvailable ? 'Скрыть' : 'Показать'}</Button>
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <Card key={order.id} className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-lg font-bold text-blue-600 mt-1">{order.total.toLocaleString('ru-RU')} ₽</p>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {order.status === 'PENDING' && (
                      <>
                        <Button size="sm" onClick={() => updateOrderStatus(order.id, 'CONFIRMED')}>Подтвердить</Button>
                        <Button size="sm" variant="outline" onClick={() => updateOrderStatus(order.id, 'CANCELLED')}>Отменить</Button>
                      </>
                    )}
                    {order.status === 'CONFIRMED' && (
                      <Button size="sm" onClick={() => updateOrderStatus(order.id, 'PROCESSING')}>В обработку</Button>
                    )}
                    {order.status === 'PROCESSING' && (
                      <Button size="sm" onClick={() => updateOrderStatus(order.id, 'SHIPPED')}>Отправить</Button>
                    )}
                    {order.status === 'SHIPPED' && (
                      <Button size="sm" onClick={() => updateOrderStatus(order.id, 'DELIVERED')}>Доставлен</Button>
                    )}
                  </div>
                </Card>
              ))}
